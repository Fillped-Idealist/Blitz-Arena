...
export default function RockPaperScissorsGame({ onComplete, onCancel }: RockPaperScissorsGameProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const metadata = [wins, draws, losses];
      const gameHash = computeHash(2, (wins * 10) + (draws * 5), timestamp, metadata);
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        const result: GameResult = {
          gameType: 2, // RockPaperScissors
          score: (wins * 10) + (draws * 5),
          timestamp,
          gameHash,
          metadata,
