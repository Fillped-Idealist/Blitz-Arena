...
interface RoguelikeSurvivalGameTestProps {
  onCancel: () => void;
}
...
interface Player {
  ...
  healingBonus: number;  // 恢复加成值（吸血鬼之触等技能加成）
}
...
const ensureIntegers = (p: Player): Player => ({
  ...
  healingBonus: Math.floor(p.healingBonus),
...
const SKILL_POOL: Skill[] = [
  ...
  },
  {
    ...
    type: 'active',
    apply: (p) => ({ ...p, healingBonus: p.healingBonus + 8 }),
    rarity: 'epic',
    color: COLORS.epic,
    icon: SKILL_ICONS.heart
  },
...
// ==================== 主组件 ====================
export default function RoguelikeSurvivalGameTest({ onCancel }: RoguelikeSurvivalGameTestProps) {
  // UI状态
        ...
        if (gameState === GameState.PLAYING) {
          ...
          if (lastRegenTimeRef.current >= 1 && player.regenRate > 0) {
            ...
            if (player.hp < player.maxHp) {
              const healAmount = Math.min(Math.floor(player.regenRate + player.healingBonus), Math.floor(player.maxHp - player.hp));
              if (healAmount > 0) {
          ...
          {gameStateRef.current === GameState.GAME_OVER && (
            <div className="space-y-3">
              <Button
                ...
                className="w-full h-12 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-600"
              >
